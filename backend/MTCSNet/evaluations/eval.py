...
class EvaluationInstance1222(object):
    # 测试类
    def __init__(self, is_save):
        self.is_save = is_save  # 是否存储预测图像
    ...
    def __call__(self, predict, truth, save_path):
        ...
        q_metric, q_list = self._PQ_DQ_SQ_(predict, truth)
        aji = self._AJI_(predict, truth)
        dice_obj, iou_obj, hausdorff_obj = self._dice_iou_hausdorff_obj_(predict, truth)
        if self.is_save:
            h, w, c = predict.shape
            save_img = np.zeros((h, w, 3))
            predict[predict > 0] = 255
        ...
        return {"DQ-F1":q_metric[0],
                "SQ":q_metric[1],
                "PQ":q_metric[2],
                "AJI":aji,
                "dice_obj":dice_obj,
                "iou_obj":iou_obj,
                "hausdorff_obj":hausdorff_obj
                }

    def _PQ_DQ_SQ_(self, pred, true, match_iou=0.5):
        """`match_iou` is the IoU threshold level to determine the pairing between
        GT instances `p` and prediction instances `g`. `p` and `g` is a pair
        if IoU > `match_iou`. However, pair of `p` and `g` must be unique
        (1 prediction instance to 1 GT instance mapping).
        ...
        if isinstance(pred, torch.Tensor):
            pred = pred.detach().cpu().numpy()  # 如果是torch先变成numpy
        ...
        # truth_iou = np.where(truth_iou > 0, np.ones_like(truth_iou), np.zeros_like(truth_iou))
        # pre_iou = self._fill_(pre_iou)  # 填充连通域的空洞——这个我就照着加了！
        true_id_list = list(np.unique(true).astype(np.int64))  # 预测列表和gt列表
        pred_id_list = list(np.unique(pred).astype(np.int64))
        assert max(true_id_list) == len(true_id_list) - 1
        assert max(pred_id_list) == len(pred_id_list) - 1  # 保证连续编号
        ...
        for t in true_id_list[1:]:
            t_mask = np.array(true == t, np.uint8)  # binary mask
            # 压扁
            t_mask = np.max(t_mask, axis=2)  # h w
            true_masks.append(t_mask.astype(np.int64))

        pred_masks = [
            None,
        ]
        for p in pred_id_list[1:]:
            p_mask = np.array(pred == p, np.uint8)  # binary mask
            # 压扁而且还需要填充
            p_mask = np.max(p_mask, axis=2)  # h w
            p_mask = self._fill_(p_mask)  # 填充连通域的空洞
            # 是否考虑一下如果出现大于一个连通域的情况清除面积较少的连通域？？？？？？？
            pred_masks.append(p_mask.astype(np.int64))
        ...
        # caching pairwise iou
        for true_id in true_id_list[1:]:  # 0-th is background  对于每一个gt
            t_mask = true_masks[true_id]
            pred_true_overlap = pred[t_mask > 0, :]  # 挖
            pred_true_overlap_id = np.unique(pred_true_overlap).astype(np.int64)
            pred_true_overlap_id = list(pred_true_overlap_id)
            for pred_id in pred_true_overlap_id:  # 对于与之重叠的每一个pred，算他们的iou
                if pred_id == 0:  # ignore
                    continue  # overlaping background
    ...
    def _AJI_(self, pred, true):
        ...
        for true_id in true_id_list[1:]:  # 0-th is background
            t_mask = true_masks[true_id]  # 对于每一个truth
            pred_true_overlap = pred[t_mask > 0, :]  # 挖一下  hw 10
            pred_true_overlap_id = np.unique(pred_true_overlap).astype(np.int64)  # 看看pred上有几个细胞和这个gt产生交集
            pred_true_overlap_id = list(pred_true_overlap_id)
            for pred_id in pred_true_overlap_id:  #
                if pred_id == 0:  # ignore
                    continue  # overlaping background
    ...
    def _dice_iou_hausdorff_obj_(self, pred, gt, hausdorff_flag=True):
        ...
        gt_id_list = list(np.unique(gt).astype(np.int64))  # 预测列表和gt列表
        pred_id_list = list(np.unique(pred).astype(np.int64))
        assert max(gt_id_list) == len(gt_id_list) - 1
        assert max(pred_id_list) == len(pred_id_list) - 1  # 保证连续编号

        gt_masks = [
            None,
        ]
        for t in gt_id_list[1:]:
            t_mask = np.array(gt == t, np.uint8)  # binary mask
            # 压扁
            t_mask = np.max(t_mask, axis=2)  # h w
            gt_masks.append(t_mask)
        ...
        for i in range(1, Ng + 1):  # 对于每一个gt
            ...
            if obj_no.size == 0:  # no intersection object  就是这个gt没有和预测产生交集
                ...
                obj_areas = [np.sum(overlap_parts == k) for k in obj_no]
                seg_obj = obj_no[np.argmax(obj_areas)]  # segmented object number  取重叠最大的来计算指标
                pred_i = pred_masks[seg_obj.astype(np.int64)]  # segmented object
