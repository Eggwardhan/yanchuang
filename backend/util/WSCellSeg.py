...
class WSCellSeg(Dataset):
    ...
    def __getitem__(self, index):
        if self.mode=="info":
            img_path = self.test_image_dir
            anno_path = self.test_image_dir+"/shit"
            gt_path = self.test_image_dir+"/shit"
            semantic_path =self.test_image_dir+"/shit"
            heat_path=self.test_image_dir+"/shit"
            point_path= self.test_image_dir+"/shit"
            vor_path=self.test_image_dir+"/shit"
        else:
        ...
        anno = cv2.imread(anno_path,flags=0) if self.mode in ['train_weak', 'train_full'] else np.ones_like(img[:,:,0])*255
        gt = tif.imread(gt_path).astype(np.int32) if os.path.exists(gt_path) else np.zeros_like(img[:,:,0]).astype(np.int32)
        ...
        if self.mode in ['train_weak', 'train_full', 'finetune', 'all_full', 'all_weak']:
            ...
            deg_dict = gen_deg_n_dist(img, point_dict, vor.copy())
            ...
            img_meta['deg_map'] = deg_map
            img_meta['seg_shape'] = [int(img_meta['ori_shape'][0]*self.test_multi_scale[0]), int(img_meta['ori_shape'][1]*self.test_multi_scale[0])]        # Take the first scale as its segmentation scale, it can adjust the img size when assigning the cell ids
